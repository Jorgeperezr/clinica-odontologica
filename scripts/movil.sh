...
#   bash scripts/movil.sh --comprobar         solo diagnostica
#   bash scripts/movil.sh --web               en Chrome, sin Xcode
...
if [ "${WEB}" -eq 1 ]; then
  verde "Chrome (--web). No hace falta Xcode ni Android Studio."
  aviso "Es para VER la app, no para entregarla: en web los tokens van a"
  echo "    almacenamiento del navegador, no al Keychain del teléfono."
...
#   · Chrome es el propio Mac                       -> localhost, pero con CORS
URL=""
if [ "${WEB}" -eq 1 ]; then
  URL="http://localhost:${PUERTO_API}"
  echo "    Chrome -> ${URL}"
  # Chrome comprueba CORS y Django solo manda `access-control-allow-origin`
  # a los orígenes de su lista. Sin el puerto web dentro, la API responde
  # 200 pero el navegador descarta la respuesta y la app se queda
  # cargando sin decir por qué: el motivo solo se ve en la consola del
  # navegador. Por eso el puerto se fija y se comprueba aquí.
  if curl -fsS -X OPTIONS "http://localhost:${PUERTO_API}/api/v1/app/me/" \
       -H "Origin: http://localhost:${PUERTO_WEB}" \
       -H "Access-Control-Request-Method: GET" -D - -o /dev/null 2>/dev/null \
       | grep -qi "access-control-allow-origin"; then
    verde "CORS permite http://localhost:${PUERTO_WEB}"
  else
    rojo "CORS NO permite http://localhost:${PUERTO_WEB}: Chrome bloqueará la app."
    echo "    Levanta la API así:"
    echo "    CORS_ALLOWED_ORIGINS=http://localhost:3000,http://localhost:${PUERTO_WEB} bash scripts/start-local.sh"
    FALLOS=$((FALLOS+1))
  fi
elif [ -n "${IP_MANUAL}" ]; then
  ...
  aviso "Y que Django escuche fuera de localhost:"
  echo "    cd django-api \&\& python manage.py runserver 0.0.0.0:${PUERTO_API}"
else
...
titulo "7. Arrancando"
if [ "${WEB}" -eq 1 ]; then
  echo "    API_URL=${URL}  ·  puerto web ${PUERTO_WEB}"
  echo
  exec flutter run -d chrome --web-port="${PUERTO_WEB}" \
